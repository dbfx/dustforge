...
export function registerRegistryCleanerIpc(mainWindow: BrowserWindow): void {
  ipcMain.handle(IPC.REGISTRY_SCAN, async (): Promise<RegistryEntry[]> => {
    ...
    // Check if telemetry is not minimized
    try {
      const { stdout } = await execFileAsync('reg', [
        'query',
      ...
      const match = stdout.match(/AllowTelemetry\s+REG_DWORD\s+0x(\d+)/i)
      if (!match || parseInt(match[1]) > 0) {
        entries.push({
          ...
          issue: 'Windows telemetry is not set to minimum — diagnostic data is being sent to Microsoft',
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
        })
      ...
      entries.push({
        ...
        keyPath: 'HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\DataCollection',
        valueName: 'AllowTelemetry',
        issue: 'Windows telemetry is at default level — diagnostic data is being sent to Microsoft',
        risk: 'low',
        selected: true,
        fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
      })
    ...
    try {
      const adKey = 'HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\AdvertisingInfo'
      const { stdout } = await execFileAsync('reg', [
        'query', adKey, '/v', 'Enabled'
      ], { timeout: 5000 })
      const match = stdout.match(/Enabled\s+REG_DWORD\s+0x(\d+)/i)
      if (!match || match[1] !== '0') {
        entries.push({
          id: randomUUID(),
          type: 'privacy',
          keyPath: adKey,
          valueName: 'Enabled',
          issue: 'Advertising ID is enabled — apps can track you with a unique identifier',
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
        })
      ...
      entries.push({
        ...
        valueName: 'Enabled',
        issue: 'Advertising ID is enabled by default — apps can track you with a unique identifier',
        risk: 'low',
        selected: true,
        fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
      })
    ...
    // Check if Activity History / Timeline is enabled
    try {
      const { stdout } = await execFileAsync('reg', [
        'query',
      ...
      const match = stdout.match(/EnableActivityFeed\s+REG_DWORD\s+0x(\d+)/i)
      if (!match || match[1] !== '0') {
        entries.push({
          id: randomUUID(),
          type: 'privacy',
          ...
          issue: 'Activity History is enabled — Windows tracks and syncs your app/file usage',
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
        })
      ...
      entries.push({
        ...
        keyPath: 'HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\System',
        valueName: 'EnableActivityFeed',
        issue: 'Activity History is enabled by default — Windows tracks and syncs your app/file usage',
        risk: 'low',
        selected: true,
        fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
      })
    ...
    // Check if Bing search is enabled in Start Menu
    try {
      const { stdout } = await execFileAsync('reg', [
        'query',
      ...
      const match = stdout.match(/DisableSearchBoxSuggestions\s+REG_DWORD\s+0x(\d+)/i)
      if (!match || match[1] !== '1') {
        entries.push({
          id: randomUUID(),
          type: 'privacy',
          ...
          issue: 'Bing web search is enabled in Start Menu — search queries are sent to Microsoft',
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '1' }
        })
    ...
    // Check if web results in Windows Search are enabled
    try {
      const searchKey = 'HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Search'
      const { stdout } = await execFileAsync('reg', [
        'query', searchKey, '/v', 'BingSearchEnabled'
      ], { timeout: 5000 })
      const match = stdout.match(/BingSearchEnabled\s+REG_DWORD\s+0x(\d+)/i)
      if (match && match[1] !== '0') {
        entries.push({
          id: randomUUID(),
          type: 'privacy',
          ...
          issue: 'Web results are enabled in Windows Search — searches are sent online',
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
        })
      ...
      // Key missing is ambiguous, skip
    }

    // Check if suggested content in Settings is enabled
    try {
      const cdmKey = 'HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager'
      const { stdout } = await execFileAsync('reg', [
        'query', cdmKey, '/v', 'SubscribedContent-338393Enabled'
      ], { timeout: 5000 })
      const match = stdout.match(/SubscribedContent-338393Enabled\s+REG_DWORD\s+0x(\d+)/i)
      if (!match || match[1] !== '0') {
        entries.push({
          id: randomUUID(),
          type: 'privacy',
          ...
          issue: 'Suggested content is enabled in Settings — Microsoft shows ads and app suggestions',
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
        })
    ...
    // Check if tips/tricks notifications are enabled
    try {
      const cdmKey = 'HKCU\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\ContentDeliveryManager'
      const { stdout } = await execFileAsync('reg', [
        'query', cdmKey, '/v', 'SubscribedContent-338389Enabled'
      ], { timeout: 5000 })
      const match = stdout.match(/SubscribedContent-338389Enabled\s+REG_DWORD\s+0x(\d+)/i)
      if (!match || match[1] !== '0') {
        entries.push({
          id: randomUUID(),
          type: 'privacy',
          ...
          issue: 'Windows tips and suggestions notifications are enabled',
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
        })
    ...
    try {
      const { stdout } = await execFileAsync('reg', [
        ...
        '/v', 'NumberOfSIUFInPeriod'
      ], { timeout: 5000 })
      const match = stdout.match(/NumberOfSIUFInPeriod\s+REG_DWORD\s+0x(\d+)/i)
      if (!match || match[1] !== '0') {
        entries.push({
          id: randomUUID(),
          type: 'privacy',
          ...
          issue: 'Windows feedback prompts are not disabled — Microsoft periodically asks for feedback',
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
        })
    ...
    try {
      ...
      const match = stdout.match(/AllowCortana\s+REG_DWORD\s+0x(\d+)/i)
      if (!match || match[1] !== '0') {
        entries.push({
          id: randomUUID(),
          type: 'performance',
          ...
          issue: 'Cortana is enabled — consumes background resources and sends data to Microsoft',
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '0' }
        })
    ...
    try {
      const { stdout } = await execFileAsync('reg', [
        'query', 'HKLM\\SYSTEM\\CurrentControlSet\\Services\\DiagTrack', '/v', 'Start'
      ], { timeout: 5000 })
      const match = stdout.match(/Start\s+REG_DWORD\s+0x(\d+)/i)
      if (match && (match[1] === '2' || match[1] === '3')) {
        entries.push({
          id: randomUUID(),
          type: 'service',
          keyPath: 'HKLM\\SYSTEM\\CurrentControlSet\\Services\\DiagTrack',
          valueName: 'Start',
          issue: `Connected User Experiences (DiagTrack) telemetry service is ${match[1] === '2' ? 'auto-start' : 'enabled'}`,
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '4' }
        })
    ...
    // Check dmwappushservice
    try {
      const { stdout } = await execFileAsync('reg', [
        'query', 'HKLM\\SYSTEM\\CurrentControlSet\\Services\\dmwappushservice', '/v', 'Start'
      ], { timeout: 5000 })
      const match = stdout.match(/Start\s+REG_DWORD\s+0x(\d+)/i)
      if (match && (match[1] === '2' || match[1] === '3')) {
        entries.push({
          id: randomUUID(),
          type: 'service',
          keyPath: 'HKLM\\SYSTEM\\CurrentControlSet\\Services\\dmwappushservice',
          valueName: 'Start',
          issue: 'WAP Push Message service is enabled — used for telemetry data routing',
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '4' }
        })
    ...
    try {
      const { stdout } = await execFileAsync('reg', [
        'query', 'HKLM\\SYSTEM\\CurrentControlSet\\Services\\MapsBroker', '/v', 'Start'
      ], { timeout: 5000 })
      const match = stdout.match(/Start\s+REG_DWORD\s+0x(\d+)/i)
      if (match && match[1] === '2') {
        entries.push({
          id: randomUUID(),
          type: 'service',
          keyPath: 'HKLM\\SYSTEM\\CurrentControlSet\\Services\\MapsBroker',
          valueName: 'Start',
          issue: 'Downloaded Maps Manager is auto-starting — unnecessary if not using Windows Maps',
          risk: 'low',
          selected: true,
          fix: { op: 'set-value', regType: 'REG_DWORD', data: '4' }
        })
    ...
    // Detect Microsoft telemetry scheduled tasks
    const telemetryTasks = [
      { path: '\\Microsoft\\Windows\\Application Experience\\Microsoft Compatibility Appraiser', label: 'Compatibility Appraiser (telemetry collector)' },
      { path: '\\Microsoft\\Windows\\Application Experience\\ProgramDataUpdater', label: 'Program Data Updater (telemetry)' },
      { path: '\\Microsoft\\Windows\\Autochk\\Proxy', label: 'Autochk Proxy (telemetry data collector)' },
      { path: '\\Microsoft\\Windows\\Customer Experience Improvement Program\\Consolidator', label: 'CEIP Consolidator (telemetry upload)' },
      { path: '\\Microsoft\\Windows\\Customer Experience Improvement Program\\UsbCeip', label: 'USB CEIP (USB telemetry)' },
      { path: '\\Microsoft\\Windows\\DiskDiagnostic\\Microsoft-Windows-DiskDiagnosticDataCollector', label: 'Disk Diagnostic Data Collector' },
      { path: '\\Microsoft\\Windows\\Feedback\\Siuf\\DmClient', label: 'Feedback DM Client (telemetry)' },
      { path: '\\Microsoft\\Windows\\Maps\\MapsUpdateTask', label: 'Maps Update Task (downloads map data)' },
      { path: '\\Microsoft\\Windows\\Maps\\MapsToastTask', label: 'Maps Toast Notification Task' }
    ]
    for (const task of telemetryTasks) {
      try {
        await execFileAsync('schtasks', ['/query', '/tn', task.path], { timeout: 5000 })
        entries.push({
          id: randomUUID(),
          type: 'task',
          keyPath: task.path,
          valueName: 'Scheduled Task',
          issue: `${task.label} is active — can be disabled to reduce telemetry/background activity`,
          risk: 'low',
          selected: true,
          fix: { op: 'disable-task' }
        })
      } catch {
        // Task doesn't exist, good
      }
    }
